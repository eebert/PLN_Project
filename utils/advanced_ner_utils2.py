...
import torch
from utils.ner_utils import extract_context_ner, is_relevant_entity, normalize_entity, normalize_entity_bert
...
def add_entity_to_list(
    ...
    # Verificar si hay tokens en la entidad actual
    if not current_entity_tokens:
        return  # Si no hay tokens, no hay nada que agregar
    ...
    if model_type == "roberta":
        ...
        print(f"Subtokens: {current_entity_tokens}, Entidad Reconstruida: '{original_entity}'")

    else:
        raise ValueError("model_type debe ser 'roberta' o 'bert'")

    normalized_entity = normalize_entity_bert(original_entity)
    print(f"Entidad antes de normalizar: '{original_entity}', después: '{normalized_entity}'")
    ...
    if not any(
        ...
        if entity_start != -1:
            #print(f"Entidad no encontrada: '{original_entity}'")
            #print(f"Oración completa: '{text}'")
            #print(f"Subtokens: {current_entity_tokens}")
            #print(f"Etiqueta de entidad: '{current_label}'")
            ...
            print(f"Contexto cercano en el texto: '{text[max(0, entity_end-50):min(len(text), entity_end+50)]}'")
            print(f"Oración completa: '{text}'")
            print(f"Etiqueta de entidad: '{current_label}'")
...
def perform_ner_with_bert(text, tokenizer, model, model_type="bert"):
    try:
        ...
        for idx, pred in enumerate(outputs.logits[0]):
            ...
            token = tokens.input_ids[0][idx]
            decoded_token = tokenizer.decode([token], skip_special_tokens=True)
            print(f"Token: '{decoded_token}', Etiqueta: '{label}'")

            if label.startswith("B-") and current_entity_tokens:
                # Procesa la entidad actual antes de empezar una nueva
                add_entity_to_list(
                    current_entity_tokens,
                    tokenizer,
                    text,
                ...
                current_entity_tokens.append(token)
            elif label == "O" and current_entity_tokens:
                # Procesa la entidad actual si no es parte de una entidad
                add_entity_to_list(
                    current_entity_tokens,
                    tokenizer,
                    text,
        ...
        if current_entity_tokens:
            # Procesa la última entidad si hay una
            add_entity_to_list(
                current_entity_tokens,
                tokenizer,
                text,
